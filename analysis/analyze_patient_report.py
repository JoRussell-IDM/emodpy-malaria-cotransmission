...
class TimeseriesAnalyzer(IAnalyzer):
    ...
    def __init__(self, filenames=[os.path.join('output', 'MalariaPatientReport.json')],
                 save_output=True):
    ...
    def map(self, data, simulation):
        tdata = data[self.filenames[0]]['patient_array']
    ...
    def plot_biting_timeseries(self,vector_to_human,human_to_vector,roots):
        ...
        plt.savefig(r'C:\git\emodpy-malaria-cotransmission\analysis\output\biting_timeseries_import.png')
        plt.show()
    ...
    def plot_gametocyte_density_by_progeny(self,vector_to_human):
        ...
        axs[0].hist(has_progeny_gams, bins=np.arange(0, max(has_progeny_gams + no_progeny_gams), 1))
        axs[1].hist(no_progeny_gams, bins=np.arange(0, max(has_progeny_gams + no_progeny_gams), 1))
        success_mean = round(np.mean(has_progeny_gams), 2)
        success_std = round(np.std(has_progeny_gams), 2)
        no_mean = round(np.mean(no_progeny_gams), 2)
        no_std = round(np.std(no_progeny_gams), 2)

        plt.yscale('log')
        axs[0].set(title=f'successful progeny (mean:{success_mean},sd:{success_std})')
        axs[1].set(title=f'no progeny (mean:{no_mean},sd:{no_std})', xlabel='gametocyte density')
        plt.savefig(r'C:\git\emodpy-malaria-cotransmission\analysis\output\gametocyte_breakdown_by_progeny_import.png')
